  ...
  */  
    ...
    public BranchGroup createSceneGraph5b() {
    	...
    	*/
    	...
    	{
    		float x= (i * xscaleFactor -  (float)ship.length/20)*shrink; 
    		for (int j=0; j<ship.height;j++) 
        	{
        		float y= (j * yscaleFactor - (float)ship.height/20)*shrink; 
    		
    			for (int k=0; k<ship.width;k++) 
            	{
            		float z= (k * zscaleFactor - (float)ship.width/20)*shrink;
            		...
            		TransformGroup transformGroup7 = new TransformGroup(transform7);
            		transformGroup7.addChild(addBox(0.1f*shrinkc, 0.1f*shrinkc, 0.1f*shrinkc, new Color3f(1, 1, 1), new Color3f(1, 1, 1),textureName));
